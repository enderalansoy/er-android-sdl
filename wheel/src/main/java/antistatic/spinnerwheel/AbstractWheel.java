    ...
    protected void initData(Context context) {
        ...
        mScroller = createScroller(new WheelScroller.ScrollingListener() {
            ...
            public void onTouchUp() {
                if (!mIsScrollingPerformed) onScrollTouchedUp(); // if scrolling IS performed, whe should use onFinished instead
            }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (Math.abs(mLastDownX - event.getX()) < 50) {
                    setCurrentItem(mCurrentItemIdx + items, true);
                    return true;
                }
                if (!mIsScrollingPerformed) {
